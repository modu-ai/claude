...
| **사전 빌트인** | Claude Design 자체 빌트인(Apple · Linear · Stripe 등) · `moai-design:design-system-library` 75개 글로벌 브랜드 토큰 | 시작점 — 이후 우리 브랜드로 커스터마이즈 |
...
{{< hint type="note" >}}
**빌트인 시스템에서 한 단계 빠르게 시작.** 자체 자산을 준비하기 전에 `moai-design:design-system-library` 스킬을 호출하면 **75개 글로벌 브랜드 디자인 시스템 토큰**을 곧바로 불러올 수 있습니다. Notion · Linear · Stripe · Vercel · Figma · Sentry · ClickHouse · Clay 등을 토큰(색·타이포·radius·spacing) 단위로 보관한 단일 진실 원천으로, Cowork에서는 `/design-system-library`로 호출합니다.
...
![Organization settings → Design systems — 디자인 시스템 등록 위치](/screenshots/claude-design/cd-ds-settings.png)

Claude Design의 조직 설정 화면에서 디자인 시스템을 등록하고 관리합니다.
...
```
이 절차는 토큰 비용을 크게 줄여 줍니다. 자체 브랜드 자산이 아직 없다면 `design-system-library`의 75개 시스템 중 브랜드 무드와 가장 가까운 것을 DESIGN.md의 출발점으로 삼으면 됩니다 — `claude-design-system-prep`가 해당 시스템 토큰을 불러와 합성합니다.
...
![디자인 시스템 Published 상태 — 활성화되면 모든 신규 프로젝트에 자동 적용](/screenshots/claude-design/cd-ds-published.png)

디자인 시스템을 Published 상태로 설정하면 조직 내 모든 사용자의 신규 프로젝트에 자동으로 적용됩니다.
...
![GitHub 저장소 연결 — 토큰 및 경로 설정](/screenshots/claude-design/cd-ds-github.png)

Claude Design이 GitHub 저장소에서 디자인 시스템 자산(마크다운, 색상 정의, 컴포넌트 메타데이터)을 읽을 수 있도록 토큰을 설정합니다.
...
## 출력 자가 검증 — 시스템에 대조 후 수정

디자인 시스템을 등록하면 Claude는 단순히 컴포넌트를 참고하는 데 그치지 않습니다. 시안을 생성한 뒤 **출력을 등록된 디자인 시스템에 스스로 대조**하고, 색·타이포·간격·컴포넌트 규칙에서 벗어난 부분을 **사용자에게 보여주기 전에 자동으로 교정**합니다. 즉 "생성 → 시스템 대조 → 자가 수정 → 표시"의 루프가 내부에서 돌기 때문에, 시스템을 제대로 등록할수록 첫 시안의 일관성이 높아집니다. 하나의 프로젝트에 여러 시스템(예: 제품 UI + 마케팅 브랜드)을 함께 등록해 두고 맥락에 맞게 적용할 수도 있습니다.

## 엔터프라이즈 거버넌스 — 표준 시스템 승인·잠금

여러 명이 같은 브랜드로 작업하는 조직에서는 시스템이 제각각 갈라질 위험이 있습니다. 이를 막기 위해 엔터프라이즈에는 **관리자(admin) 역할**이 있습니다. 관리자는 **표준 디자인 시스템 1개를 승인**하고 **편집을 잠글** 수 있어, marketing·product·design 팀이 만드는 AI 산출물이 승인된 타이포·색·간격·컴포넌트 규칙에서 벗어나지 않도록 강제합니다. 자세한 설정은 [Claude Design admin 가이드](https://support.claude.com/en/articles/14604406-claude-design-admin-guide-for-team-and-enterprise-plans)를 참고하세요.
