...
1. **스타일이 입혀진 HTML 리포트** (예: `moai-coworker:office-html-report`·`moai-coworker:office-html-slide` 산출물,
...
그대로 렌더합니다. `moai-coworker:office-html-report`·`moai-coworker:office-html-slide` 산출물을 그대로 넘기면
...
| 스킬 | 관계 | 사용 시점 |
|------|------|-----------|
| `moai-coworker:office-html-report` | before (HTML 리포트 생성 → 이 스킬로 PDF 변환) | "리포트 만들고 PDF로도" 흐름 |
| `moai-coworker:office-html-slide` | before (HTML 슬라이드 생성 → PDF 변환) | 슬라이드 덱을 PDF 배포본으로 |
| `moai-coworker:office-docx-generator` | alternative (편집 가능한 Word 산출물) | 수신자가 편집 가능 파일 필요 시 |
