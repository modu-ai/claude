...
`moai-marketer:media-higgsfield-image` 스킬 또는 Higgsfield MCP 툴 직접 호출. 11개 모델을 API 키 하나로 통합 제공.
...
moai-marketer:media-gpt-image-2-prompt → 산출 프롬프트 → media-higgsfield-image(GPT Image 2 모델)
```
...
`moai-marketer:media-codex-image` 스킬 또는 codex CLI의 `image_gen` 도구 직접 호출. 최초 1회 `codex login` 필요.
