...
description: |
  ...
  [책임 경계] vs moai-cowork:html-report: 이 스킬=도식·차트·코드가 풍부한 학습자료(JS 라이브러리 허용), 저 스킬=0-JS 단일파일 업무 보고서.
version: 0.1.0
---
...
- `:root` 디자인 토큰은 `moai-cowork:html-report`의 `references/design-tokens.md`를 그대로 사용(Anthropic 영감 팔레트).
...
- **moai-cowork:tutor-research**: 이 스킬의 입력 종합본을 만든다
- **moai-cowork:learning-project**: 생성된 자료를 materials/에 보관하고 진도를 갱신한다
