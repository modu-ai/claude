...
office-html-slide는 편집 가능 `.pptx`를 **직접 생성하지 않습니다**. `moai-officer:office-pptx-designer` 체이닝으로 위임합니다. office-pptx-designer가 이미 pptxgenjs + HTML-First + 9 아키타입 + QA 파이프라인을 보유하고 있어, office-html-slide가 자체 구현하면 중복·책임 모호화가 발생하기 때문입니다.
...
따라서 office-html-slide가 산출한 `deck.json` 원고를 SSOT로 office-pptx-designer에 전달하면, office-pptx-designer의 pptxgenjs가 **원고 → OOXML 객체를 직접 생성**합니다:

```
deck.json 원고 ─┬─→ HTML 렌더 (office-html-slide)
                └─→ PPTX 렌더 (office-pptx-designer, pptxgenjs)
...
office-html-slide의 design_system(75 시스템) → office-pptx-designer의 10 큐레이션 팔레트 자동 매핑 규약:

| office-html-slide design_system | office-pptx-designer 팔레트 | 근거 |
|--------------------------|----------------------|------|
...
| 그 외 75개 | Claude Classic (기본 폴백) + 시스템 primary 색을 office-pptx-designer primary에 주입 | design-system-library 토큰 우선 |

> 매핑이 완벽하지 않아도 office-pptx-designer가 자체 10팔레트로 일관된 PPTX를 산출합니다. design_system 토큰의 primary/canvas 색을 office-pptx-designer의 colors 객체에 주입해 최대한 브랜드 정합.

## chart-data → 네이티브 차트 매핑 (편집 가능성 보증)

`deck.json`의 `chart` 필드가 있으면 office-pptx-designer가 **네이티브 차트**로 생성합니다 (이미지가 아님):
...
유일한 신뢰 경로 = **deck.json 원고 SSOT에서 HTML·PPTX 병행 생성** (office-html-slide + office-pptx-designer 체이닝).

## 체이닝 호출 패턴

```
# 1. office-html-slide가 deck.json + deck.html 산출
# 2. 사용자가 export_pptx 요청 시 office-pptx-designer에 deck.json 전달
"office-pptx-designer 스킬로 이 deck.json 원고를 편집 가능 .pptx로 렌더해줘.
 design_system은 claude(Claude Classic 팔레트) 승계,
 chart.data는 네이티브 차트로 매핑."
# 3. office-pptx-designer가 pptxgenjs로 .pptx 산출 + QA
# 4. office-html-slide가 office-pptx-designer QA 결과를 통합 보고
```

## 한국어 폰트 처리

office-pptx-designer theme으로 한국어 표준 폰트 지정:
```javascript
