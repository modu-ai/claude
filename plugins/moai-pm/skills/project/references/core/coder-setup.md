...
> **4-plugin 허브 라우터의 코더 분기 정본.** 개발 프로젝트에 **MoAI-ADK 3.0 정본 baseline**(`.claude/`·`.moai/`·`CLAUDE.md`·`.mcp.json`)을 `moai`(구 moai) 플러그인의 `moai-workflow-project` 정본(templates 11종)으로 설치·구성한다. SPEC DDD/TDD 방법론과 TRUST 5 품질 게이트를 프로젝트에 설정한다. PM 공통 CLAUDE.md.tmpl은 이 분기에 **적용하지 않는다**(두-템플릿 분리 — Phase 3-1).
...
모든 스킬은 **`moai` 단일 플러그인**(구 moai — 개명 후에도 코더 분기 담당은 동일) 소속이다.
