...
weight: 1
description: "프로젝트 초기화 허브 AI 직원. /project 한마디로 12개 AI 직원 플러그인의 진입을 라우팅합니다."
...
PM은 두 가지 모드로 일합니다. `--cowork` 모드(기본값 — 플래그를 생략하면 이 모드)는 개발을 제외한 모든 협업 프로젝트용으로, goose 스킬이 전 직원 지침 설정과 `.claude/agents/` 커스텀 에이전트, CLAUDE.md(클로드가 프로젝트마다 읽는 지침 파일) 워크플로우까지 생성하고 스스로 개선해 나갑니다. `--code` 모드는 코더의 개발 환경 셋업(`moai init` 동등 효과)을 담당할 예정으로, **현재 개발 중**입니다. 비개발자도 자연어 한마디로 프로젝트를 시작할 수 있게 하는 것이 존재 이유입니다.

스킬은 단 하나(`project`)뿐이지만, 이 하나가 나머지 14명의 직원을 배치하는 관문 역할을 합니다. 그래서 팀의 "허브"라고 부릅니다.

```mermaid
flowchart LR
    A["요청<br/>(프로젝트 시작)"] --> B["모드 판별<br/>--cowork / --code"]
    B --> C["직원 지침 배치"]
    C --> D["CLAUDE.md 생성"]
    D --> E["산출물<br/>(작업 준비 완료)"]
```
...
**3. 개발 환경 부트스트랩 (개발 중).** "코딩 프로젝트 셋업해줘"에 답하는 `--code` 모드 — 코더의 개발 방법론(SPEC plan/run/sync) 환경을 구성하는 기능 — 는 현재 개발 중이며, 코더 플러그인 배포와 함께 열립니다. 그 전까지는 `--cowork` 모드를 사용해 주세요.
