...
**body 스키마 필드**가 한국어로 포함되어 있어, MCP 클라이언트가 별도 문서 없이도
action 을 선택하고 파라미터를 구성할 수 있습니다.

## 설치

`.mcp.json`(`plugins/moai-coworker/.mcp.json`)에 등록:

```jsonc
"moai-imweb": {
  "command": "uv",
  "args": ["run", "--directory", "./mcp-servers/moai-imweb", "moai-imweb-mcp"],
  "env": {
