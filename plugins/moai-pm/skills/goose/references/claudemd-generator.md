# claudemd-generator.md — CLAUDE.md 생성 프로토콜 (goose Desktop 변형)

## 개요

`/goose --project` Phase 6에서 호출되는 `CLAUDE.md` 자동 생성 프로토콜. 사용자 맞춤형 `./CLAUDE.md`를 생성하되, **200라인 이내**로 제한한다. 스킬 상세 내용은 `CLAUDE.md`에 복사하지 않고, 실행 시 해당 스킬(SKILL.md)을 런타임에 로드하여 사용한다.
...
Phase 3에서 설계된 각 산출물 체인을 `{workflow_chains}` 슬롯에 인라인 스킬 체인으로 주입한다. 최대 **10개 체인까지** 나열한다. 나머지 체인은 `/goose catalog`로 참조 유도한다.
...
| 상황 | 동작 |
|------|------|
| `/goose --project` 재실행 | `CLAUDE.md` 재생성(재진입 확인 후 — SKILL.md §Socratic Interview S3 참조) |
| `/project evolve` | 자가 개선 진단을 `.moai/evolution/`에 기록(`CLAUDE.md`는 `<!-- evolution-log -->`만 갱신) |
| 플러그인 추가 설치 | `/goose --project` 재실행 권장(체인 재설계) |
