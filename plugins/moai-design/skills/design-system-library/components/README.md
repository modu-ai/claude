...
shadcn UI 컴포넌트를 React 없이 **vanilla HTML + Tailwind utility class**로 재현한 참조 마크업입니다. 단일 파일 HTML 산출물(office-html-report 등)에서 `design_system` 토큰과 함께 카드·버튼·테이블 등을 칠할 때 참조합니다. 토큰 규칙은 [`../mapping/tailwind.md`](../mapping/tailwind.md) §1·§3 기준.
...
- 0의존 self-contained 출력이 필요한 경우(이메일 첨부 · 오프라인 · 인쇄)는 `design_system`을 지정하지 않고 기존 office-html-report 0의존 템플릿을 사용하세요
