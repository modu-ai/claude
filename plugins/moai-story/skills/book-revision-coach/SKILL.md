...
description: |
  도서 본문 퇴고·교열 코치 — 7 단계 점검(어법·문체·논리·인용·분량·시각자료·일관성)으로 출판 직전 원고를 다듬는 스킬. 국립국어원 어문규범 기준 + 한국 출판사 편집자 관점 + book-chapter-writer의 4 장르 문체 일관성 검증. office-korean-spell-check(맞춤법) + general-humanize-korean(AI 티) + general-ai-slop-reviewer(최종) 체인의 첫 단계. 분량 정리·인용 정합성·문체 통일·논리 흐름 검증 후 출판사 투고 준비 완료 상태로 출력.
...
> 💡 본 스킬은 1차 점검만 수행. 정밀 맞춤법은 `moai-cowork:office-korean-spell-check`(바른한글) 체이닝 권장.
...
| 항목 | 상태 | 비고 |
|------|:----:|------|
| 맞춤법 | ✅ / ⚠️ | ___ 건 (정밀 점검은 office-korean-spell-check) |
| 띄어쓰기 | ✅ / ⚠️ | ___ 건 |
...
- **다음 단계**: 수정 후 moai-cowork:general-ai-slop-reviewer → moai-cowork:general-humanize-korean 체이닝
```
...
→ Step 1: 어법·맞춤법 1차 (2건 검출, 정밀은 office-korean-spell-check)  
...
| **맞춤법만 점검** | 어법은 1차만, 정밀은 office-korean-spell-check |
| **AI 티 무시** | `moai-cowork:general-ai-slop-reviewer` → `moai-cowork:general-humanize-korean` 체이닝 필수 |
...
1차 어법·문체·논리 (book-revision-coach) ← 본 스킬
  ↓
정밀 맞춤법 (moai-cowork:office-korean-spell-check, 바른한글)
  ↓
AI 슬롭 검수 (moai-cowork:general-ai-slop-reviewer) ← 필수
  ↓
AI 티 윤문 (moai-cowork:general-humanize-korean) ← 필수
  ↓
출판사 투고용 제안서 (book-proposal-writer)
```

### moai-cowork 연계 및 미설치 폴백

위 검수 체인의 정밀 맞춤법·AI 슬롭 검수·AI 티 윤문 단계는 `moai-cowork` 플러그인 스킬과 연계된다. **moai-cowork 플러그인이 설치된 경우** `office-korean-spell-check` → `general-ai-slop-reviewer` → `general-humanize-korean` 체인을 자동 연계합니다. **미설치 시 본 스킬의 자체 퇴고 절차를 따릅니다** — Step 1(어법 1차)·Step 2(문체 일관성)·Step 7(일관성 최종) 점검과 본문 내 자체 가이드라인만으로 출판 직전 수준까지 다듬습니다.
...
### After (체이닝 — 정규 체인)
- `moai-cowork:office-korean-spell-check` — 정밀 한국어 맞춤법
...
> 퇴고 완료 본문은 정규 체인 **`book-revision-coach → moai-cowork:general-ai-slop-reviewer → moai-cowork:general-humanize-korean`**으로 마무리합니다.

### Alternative
- `moai-cowork:office-korean-spell-check` 단독 — 빠른 맞춤법만
- `moai-cowork:general-ai-slop-reviewer` 단독 — 빠른 AI 티 검출만
...
| 바른한글 | (부산대 + 나라인포테크) | 맞춤법 검사 (office-korean-spell-check 사용) |
...
- v2.9.0 (2026-05-17): 신규 스킬 도입. moai-cowork 플러그인 8번 (마지막) 워크플로우. 7 단계 점검(어법·문체·논리·인용·분량·시각자료·일관성) + 한국 출판사 편집자 관점 + 4 체인 검수 순서.
