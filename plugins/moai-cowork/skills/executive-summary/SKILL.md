...
description: |
  ...
  기본 출력은 moai-cowork:html-report로 단일 HTML(이미지·CSS·JS 인라인, 카톡·이메일 바로 공유)이며, pdf/docx/pptx/hwpx 변환은 옵션 체이닝. 입력 가능: moai-cowork:performance-report 출력 · moai-cowork:financial-statements · moai-cowork:variance-analysis · moai-cowork:pm-weekly-report · 외부 보고서. 한국 임원/이사회 표준 What/So What/Now What + K-IFRS 재무 지표 우선.
  [책임 경계] vs moai-cowork:performance-report: executive-summary=임원 압축 요약(≤500단어), performance-report=마케팅 풀 리포트(전체).
version: 0.1.0
---
...
본 스킬의 **기본 출력은 마크다운 + `moai-cowork:html-report` 렌더링한 단일 HTML 파일**입니다.
