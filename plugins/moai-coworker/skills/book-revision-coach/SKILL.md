...
> 💡 본 스킬은 1차 점검만 수행. 정밀 맞춤법은 `moai-coworker:office-korean-spell-check`(바른한글) 체이닝 권장.
...
- **다음 단계**: 수정 후 moai-coworker:general-ai-slop-reviewer → moai-coworker:general-humanize-korean 체이닝
```
...
| **AI 티 무시** | `moai-coworker:general-ai-slop-reviewer` → `moai-coworker:general-humanize-korean` 체이닝 필수 |
...
1차 어법·문체·논리 (book-revision-coach) ← 본 스킬
  ↓
정밀 맞춤법 (moai-coworker:office-korean-spell-check, 바른한글)
  ↓
AI 슬롭 검수 (moai-coworker:general-ai-slop-reviewer) ← 필수
  ↓
AI 티 윤문 (moai-coworker:general-humanize-korean) ← 필수
  ↓
...
위 검수 체인의 정밀 맞춤법·AI 슬롭 검수·AI 티 윤문 단계는 `moai-coworker` 플러그인 스킬과 연계된다. **moai-coworker 플러그인이 설치된 경우** `office-korean-spell-check` → `general-ai-slop-reviewer` → `general-humanize-korean` 체인을 자동 연계합니다. **미설치 시 본 스킬의 자체 퇴고 절차를 따릅니다** — Step 1(어법 1차)·Step 2(문체 일관성)·Step 7(일관성 최종) 점검과 본문 내 자체 가이드라인만으로 출판 직전 수준까지 다듬습니다.
...
### After (체이닝 — 정규 체인)
- `moai-coworker:office-korean-spell-check` — 정밀 한국어 맞춤법
...
> 퇴고 완료 본문은 정규 체인 **`book-revision-coach → moai-coworker:general-ai-slop-reviewer → moai-coworker:general-humanize-korean`**으로 마무리합니다.

### Alternative
- `moai-coworker:office-korean-spell-check` 단독 — 빠른 맞춤법만
