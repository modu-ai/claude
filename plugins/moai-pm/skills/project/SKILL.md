...
version: "0.3.0"
---
<!-- moai-pm v0.3.0 · 4-plugin 허브 라우터 · Fable 5 행동 패턴 증류 반영(SPEC-MOC-PM-REDESIGN-001) -->
...
| `moai` | **코더** | 개발 전담 (SPEC DDD/TDD·품질 게이트·MoAI-ADK 정본) | 28 |
| `moai-pm` | **PM** | 프로젝트 초기화 허브 (본 스킬 — 라우팅만) | 1 |

**컨셉**: 코워커는 실무+작가 모자 교체, 디자이너·코더는 전문 분리, PM은 진입 허브.

> 스킬 수는 작성 시점 스냅샷이다 — 실측 정본은 각 분기 Phase 2 인벤토리(`~/.claude/plugins/` 동적 스캔). 카운트를 하드코딩 근거로 사용하지 않는다.
...
| `/project --code` | 개발 환경 셋업 | moai | `references/core/coder-setup.md` |
...
/plugin install moai       # 개발 분기 (필요 시)
```
...
| `cowork-setup.md` | `--cowork` 분기 정본 (8-Phase·코워커 체인·역할 감지·맥락 분석 입력·인용·저작권 가드) |
| `designer-setup.md` | `--designer` 분기 정본 (브랜드 자산 합성·DESIGN.md·파일 생성 기준·맥락 적용 규칙) |
| `coder-setup.md` | `--code` 분기 정본 (MoAI-ADK 3.0 baseline 설치·두-템플릿 분리·SPEC 워크플로우) |
| `init-protocol.md` | 관리 커맨드(resume/catalog/status/apikey/doctor) + 8-Phase 상세 레퍼런스 |
| `context-collector.md` | 맥락 수집 등급(A/B/C)·심화 인터뷰 기준·맥락 적용 규칙(메타 코멘터리 금지) |
| `claudemd-generator.md` | CLAUDE.md 변수 치환·200라인 예산·HARD 규칙 블록(office 우선·ai-slop + 요청 평가 사다리·파일 생성 기준·인용·저작권 가드·톤 규칙·맥락 적용 규칙) |
| `execution-protocol.md` | 스킬 체인 순차 실행·단계별 요약·검색 스케일링 |
