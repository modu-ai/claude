# diagnostic-protocol.md — 진단 프로토콜 (goose)

## 개요

goose 환경과 프로젝트 설정 상태를 진단하고 문제를 식별하는 프로토콜이다. `/goose doctor`와 `/goose status` 명령어로 실행된다. 플러그인 인벤토리 카운트는 하드코딩하지 않는다 — `.claude-plugin/marketplace.json`이 로스터 정본이다.

---

## 1. `/goose doctor` — 환경 체크
...
│ ├─ skills/goose/references/: ✓ (references + templates 포함)
...
```
/goose status --detailed
/goose status --plugins
/goose status --export=json
```
...
IF CLAUDE.md 없음 OR "프로젝트 개요" 섹션 부재:
  ERROR: "프로젝트가 초기화되지 않았습니다. /goose --project 실행 필요"

IF 설치 플러그인 == 0:
  ERROR: "설치된 직원 플러그인이 없습니다. 마켓플레이스 설치 필요"

IF marketplace.json 로스터 밖 moai-* 플러그인 감지:
  WARNING: "로스터 밖 플러그인이 있습니다. /goose catalog 확인"

IF 스킬 체인 내 미설치 스킬 참조:
  WARNING: "스킬 체인이 미설치 스킬을 참조합니다. /goose catalog 확인"

IF 커넥터 API 키 누락:
  INFO: "일부 커넥터 키가 없습니다. /goose apikey 로 등록"
```
...
**부분 재설정**: `CLAUDE.md`만 재생성(스킬 체인 재설계) — `/goose --project` 재실행. 커넥터 키만 재등록 — `/goose apikey`.

**전체 재설정**: `/goose --project` 재실행(Phase 1 인터뷰 재실행, S3 재진입 확인 후 진행).
