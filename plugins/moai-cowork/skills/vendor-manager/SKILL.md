...
```
vendor-manager → moai-cowork:ai-slop-reviewer → moai-cowork:humanize-korean
```

- `moai-cowork:ai-slop-reviewer` — 서술형 본문의 AI 티 패턴을 검수·수정합니다.
- `moai-cowork:humanize-korean` — 검수된 본문을 자연스러운 한국어 보고체로 다듬습니다.

벤더 평가표, 리스크 평가 매트릭스 등 **점수·표 산출물**은 산문 후처리 대상이 아니며 `moai-cowork:xlsx-creator` 또는 `moai-cowork:data-visualizer`로 라우팅합니다.
