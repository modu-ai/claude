...
description: |
  ...
  [책임 경계] vs moai-coworker:media-notebooklm-slide-prompt: 이 스킬=지금 바로 열리는 .pptx 파일, 저 스킬=NotebookLM에 넣을 슬라이드 생성 프롬프트.
version: "5.0.0"
---
...
3. 색 일관성 (한 팔레트 안에서만)
4. AI 슬롭 카피 검출 (`moai-coworker:general-cd-slop-check` 체이닝)
5. 청중·톤 적합성 (사람 검수)
...
| 스킬 | 사용 시점 |
|---|---|
| `moai-coworker:office-docx-generator` | DOCX 문서 생성 (같은 Claude 톤 시스템 공유) |
| `moai-coworker:office-hwpx-writer` | 한컴 한글 문서 |
| `moai-coworker:office-xlsx-creator` | 엑셀 데이터 시트 (차트 데이터 소스) |
| `moai-coworker:office-pdf-writer` | 다국어 PDF 변환 |
| `moai-coworker:content-copywriting` | 슬라이드 카피 작성 |
| `moai-coworker:general-humanize-korean` | 한국어 카피 자연화 |
| `moai-coworker:general-cd-slop-check` | 슬라이드 카피 AI 슬롭 검수 |
| `moai-coworker:general-cd-prompt-builder` | Claude Design에 동시 시안 요청 시 |
| `moai-coworker:media-gemini-3-image-prompt` | 슬라이드 일러스트·배경 이미지 프롬프트 |
