...
- **before**: `moai-coworker:finance-investor-relations` — IR 자료 작성 시 KRX 공식 시세 필요
- **before**: DART MCP (`moai-coworker/.mcp.json`) — 공시·재무 분석과 결합
- **after**: `moai-coworker:finance-variance-analysis` — 시세 변동 분석
- **after**: `moai-coworker:office-xlsx-creator` — 시세 데이터 엑셀화
