...
description: |
  ...
  프롬프트가 복잡하거나 한국어 텍스트가 들어가면 `moai-marketer:media-gpt-image-2-prompt`(6-Block 프롬프트 빌더)로 먼저 프롬프트를 빌드한 뒤 이 스킬로 생성하세요. Higgsfield MCP가 연결돼 있지 않거나 로컬 개발·ChatGPT 구독 한도 재사용이 목적이면 이 스킬을 사용합니다 (프로덕션·CI·멱등은 `media-higgsfield-image` 권장).
version: "0.1.0"
---
...
> 한국어 텍스트 정확도 규칙은 `moai-marketer:media-gpt-image-2-prompt`의 `references/text-rendering.md`와 동일한 원칙을 따릅니다.
...
```
사용자 자연어 → moai-marketer:media-gpt-image-2-prompt (6-Block 프롬프트 빌드)
                    ↓ 산출: 6-Block 자연어 단락
              moai-marketer:media-codex-image (해당 프롬프트로 codex exec 호출 → gpt-image-2 생성)
```
