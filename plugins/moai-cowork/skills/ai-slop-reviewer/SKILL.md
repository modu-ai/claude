...
> 탐지 원본: `plugins/moai-code/skills/moai-domain-humanize/modules/korean.md` D카테고리("X에서 Y로" 변환 공식) + J카테고리(em-dash 남용). 본 게이트가 이 3종을 잡지 못하면 커머스/카피 스킬 체인(본 스킬만 호출)을 통과한 채 slop 카피가 배포됩니다.
...
### 선행 스킬 (입력 생성)
- `moai-cowork:blog` — 블로그 포스트 생성
- `moai-cowork:landing-page` — 랜딩 페이지 카피라이팅
- `moai-cowork:newsletter` — 뉴스레터 작성
- `moai-cowork:copywriting` — 마케팅 카피 작성
- `moai-cowork:docx-generator` — DOCX 문서 생성
- `moai-cowork:pptx-designer` — PPTX 발표자료 생성

### 후속 스킬 (선택적)
- `moai-cowork:higgsfield-image` — (선택) 블로그 발행 체인의 이미지 생성 단계
