...
이 섹션은 과거 "플러그인 카탈로그"(chat / cowork / design / code 4-카테고리 시절)를 대체합니다. `moai-cowork` 단일 통합 플러그인(171스킬)은 v6.0.0에서 전문가 플러그인으로 분화되었고, v6.2.0에서 스토리·미디어·데이터 애널리스트가 추가되어 17종 패밀리로 자리 잡았으며, 예전 하위 페이지 URL(`/plugins/cowork/` 등)은 모두 이 페이지로 리다이렉트됩니다. 옛 문서에서 보던 스킬들은 이름이 바뀌지 않은 채 담당 직원 플러그인으로 이사했다고 생각하면 됩니다.
