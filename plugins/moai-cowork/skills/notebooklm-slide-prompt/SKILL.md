...
요약: **NotebookLM에 넣을 준비물(소스·대본·구조·이미지 프롬프트)을 만드는 빌더**가 이 스킬입니다. 실제 `.pptx` 파일이 필요하면 `moai-cowork:pptx-designer`를 호출하세요. "PPT 파일로 뽑아줘"는 `moai-cowork:pptx-designer`, "NotebookLM 슬라이드 프롬프트 만들어줘"는 이 스킬입니다.
...
체인 종료 직전 `moai-cowork:ai-slop-reviewer`를 호출해 클리셰·번역투를 제거한다.
...
notebooklm-slide-prompt
  ↓
moai-cowork:ai-slop-reviewer
```

본 스킬은 텍스트 산출물이므로 체인 종료 직전 `moai-cowork:ai-slop-reviewer`로 후처리한다.
...
- Google AI Studio
- Vertex AI
- (MoAI 사용자) `moai-cowork:higgsfield-image` 스킬에 그대로 전달
