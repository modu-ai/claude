...
> 카피가 위 3종 중 하나라도 포함하면 게이트 FAIL → 카피 재작성 후 재검수. `ai-slop-reviewer`와 `humanize-korean` 둘 다 이 3종을 체크리스트에 등록했으므로 어느 쪽이든 잡아냅니다. 시각 검수(5단계) 전에 이 카피 전용 게이트를 먼저 통과해야 합니다.
