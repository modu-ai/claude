...
name: project
description: |
  18개 AI 직원 플러그인 패밀리('MoAI-Claude, 모두의 클로드')의 진입을 분기하는 **허브 라우터 스킬**.
  진입 모드는 두 가지뿐이다: `/project --cowork` (code를 제외한 모든 클로드 코워크 프로젝트 — 실무·작가·스토리·마케터·미디어·셀러·사무관·데이터 애널리스트·법무·재무·인사·CS·컨설턴트·커리어·튜터·디자이너 전 직원의 프로젝트 지침 설정 + `.claude/agents/` 커스텀 에이전트와 CLAUDE.md 스킬&에이전트 워크플로우 생성 + 사용 중 재귀적 자가 개선)과 `/project --code` (코더 개발환경 셋업). PM 자체는 구현 로직을 품지 않고 각 플러그인의 셋업 프로토콜로 위임한다.
...
version: "0.4.0"
---
<!-- moai-pm v0.4.0 · 18-plugin 패밀리(v6.2) · 2-모드 라우터(--cowork/--code) + 재귀적 자가 개선 (skill-regrouping 2026-07-10) -->
...
> - 🧑‍💼 **코워커** — 범용 실무 + 라이프스타일 (전략·제안서·협상·목표·습관·여행·웰니스)
> - ✍️ **작가**(출판) · 🎬 **스토리**(웹툰/웹소설/IP) · 📣 **마케터**(캠페인/콘텐츠) · 🎨 **미디어**(이미지/영상/오디오)
> - 🛒 **셀러** · 🗂️ **사무관**(오피스 문서) · 📊 **데이터 애널리스트**(공공데이터/시각화)
> - ⚖️ **법무** · 💰 **재무·세무** · 🤝 **인사·채용** · 🎧 **CS매니저** · 🧭 **컨설턴트** · 🎯 **커리어코치** · 🎓 **튜터** — 전문직 7종
...
마켓플레이스 **'MoAI-Claude, 모두의 클로드'** (`modu-ai/claude`) 소속 18개 AI 직원 (v6.2.0, 스킬 233종).

| 플러그인 | 한글명 | 역할 | 모드 |
|----------|--------|------|------|
| `moai-coworker` | 코워커 | 범용 비즈니스 실무 + 라이프스타일·자기계발 | `--cowork` |
| `moai-writer` | 작가 | 출판 기획·집필·제안서 (book-*) | `--cowork` |
| `moai-story` | 스토리 크리에이터 | 웹툰·웹소설·시나리오·콘티·IP (story-*) | `--cowork` |
| `moai-marketer` | 마케터 | 캠페인·콘텐츠 (marketing-*/content-*) | `--cowork` |
| `moai-media` | 미디어 크리에이터 | 이미지·영상·오디오 생성 (media-*) | `--cowork` |
| `moai-seller` | 셀러 | 이커머스 (스마트스토어·아임웹·카페24 MCP) | `--cowork` |
| `moai-officer` | 사무관 | 오피스 문서 (HWPX·DOCX·XLSX·PPTX·PDF·HTML) | `--cowork` |
| `moai-analyst` | 데이터 애널리스트 | 공공데이터 조회·데이터 시각화 | `--cowork` |
...
| `moai-cs` | CS매니저 | 고객지원·CRM·VOC 분석 | `--cowork` |
| `moai-consultant` | 컨설턴트 | 사업계획서·시장분석·경영 진단 | `--cowork` |
| `moai-career` | 커리어코치 | 이력서·면접·이직 (구직자 편) | `--cowork` |
...
> **폐기 플래그**: `--designer`·`--writer`·`--story`·`--marketer`·`--media`·`--officer`·`--analyst` 등 개별 직원 플래그는 v0.4.0에서 `--cowork`로 흡수되었다. 입력되면 경고 없이 `--cowork`로 처리하되 해당 직원을 워크플로우 설계의 중심에 둔다 (18개 직원 모두 `--cowork` 커버리지 안). `--code` 만이 별도 모드다.
...
/plugin install moai-coworker    # 범용 실무 + 라이프스타일 코어 (대다수 사용자)
# 이후 필요한 전문가 직원을 추가 설치: moai-writer / moai-story / moai-marketer /
# moai-media / moai-seller / moai-officer / moai-analyst / moai-lawyer /
# moai-accountant / moai-recruiter / moai-cs / moai-consultant / moai-career /
# moai-tutor / moai-designer / moai(코더)
```
...
모든 스킬 참조는 `moai-{coworker,writer,story,marketer,media,seller,officer,analyst,lawyer,accountant,recruiter,cs,consultant,career,tutor,designer}:` 접두어를 사용한다. router.md의 매핑이 단일 진실 원천이다.
