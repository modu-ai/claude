...
**단순 요청 원칙**: 세금 요율 확인, 단일 문서 생성 등 단순 요청은 이 프로토콜 전체를 따르지 않고 즉시 답변합니다. 멀티스텝 작업만 전체 워크플로우를 활성화합니다. 이 원칙은 생성 CLAUDE.md의 **요청 평가 사다리**(대화형 답변 → 스킬 체인 → 파일 생성)와 같은 판단 순서다 — 사다리 1단(대화형 답변)에서 끝나는 요청은 아래 4단계에 진입하지 않는다.
...
[단계 1] 소크라테스 인터뷰 (Interview)
  ...
  누락 정보 식별 → 열린 질문으로 확인
  맥락 충분까지 질문 (피로 방지 위해 AskUserQuestion 1회 4질문 max, 필요 시 라운드 반복)
    ↓
...
**질문 규칙:**
- **맥락 충분까지 질문** (AskUserQuestion 1회 4질문 max, 필요 시 라운드 반복 + 텍스트 대화 보충)
...
[단계별 계획]
  1. {스킬/도구 A} — {작업 내용} (예상 ~10초)
  2. {스킬/도구 B} — {작업 내용} (예상 ~30초)
  3. {검증 루프} — {검증 항목}
  4. 최종 산출물: {파일/형식/위치}
...
| 검증/평가 단계 | 생성 후 **순차** |
| 복수 플러그인 연계 (moai-coworker → moai-coworker) | **순차** |
...
```
"사용자가 방향을 잡고, MoAI가 스킬 체인으로 실행합니다.
...
### Office 문서 심층 검증 (moai-coworker 전용)

moai-coworker 스킬(office-hwpx-writer, office-docx-generator, office-pptx-designer, office-xlsx-creator)이
...
### 6-5. 검색 스케일링 (외부 사실 확인 — 도구 호출 규모)

<!-- @MX:NOTE: [AUTO] 검색 스케일링은 §6-4 검증 깊이 사다리의 보완 축이다 — 6-4가 "얼마나 깊게 검증할 것인가"를 정하면, 6-5는 "몇 번 찾을 것인가"를 정한다. 두 표의 등급 정렬(QUICK↔1회, NORMAL↔3-5회, DEEP↔5-10회+)을 깨뜨리지 말 것 -->

체인 실행이 외부 사실 확인(WebSearch/WebFetch)을 요구할 때, 도구 호출 횟수는 요청 복잡도에 비례한다. §6-4가 검증을 **얼마나 깊게** 할지 정한다면, 이 절은 **몇 번 찾을지**를 정한다.

| 요청 유형 | 호출 규모 | §6-4 정렬 |
|-----------|----------|-----------|
| 단일 사실·시세·현직 확인 (예: "부가세 신고 기한", "환율") | **1회** | QUICK |
| 비교·정책·검증 필요 주제 (예: 지원사업 요건 대조, 경쟁사 비교) | **3-5회** | NORMAL |
| 심층 리서치·다출처 종합 (예: 시장조사 리포트, 규제 분석) | **5-10회+** | DEEP |
| 확립된 개념·역사적 사실·불변 정의 (예: "복리 계산 원리") | **검색 생략** — 내부 지식으로 답변 | QUICK 이하 |
| 최신성이 결과를 좌우하는 정보 (시세·인물 현직·미인지 제품·마감일) | **반드시 검색** — 내부 지식 단정 금지 | 유형별 규모 적용 |

적용 규칙:
- 검색 결과 수치·날짜·규정은 §3.5 Layer 2.5 근거 게이트를 그대로 통과해야 한다 (출처 확보 또는 「확인 필요」 표시)
- 첫 1-2회 검색으로 답이 확정되면 남은 예산을 소진하지 않는다 — 규모는 상한 지침이지 의무 횟수가 아니다
- 검색 과정을 사용자에게 장황하게 서술하지 않는다 — 결과와 출처만 전달한다
