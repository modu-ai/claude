...
| Supervisor | 다수 전문 스킬 오케스트레이션 | moai-cowork:ai-diagnostic, project |
...
신규 스킬에 한국어 예시 카피를 넣을 때 AI 슬롭 패턴이 예시로 전염되지 않도록 아래 3종을 금지합니다. 이 규칙은 `ai-slop-reviewer`·`humanize-korean` 게이트가 슬롭으로 분류하는 패턴이 "좋은 예"로 등록되는 것을 원천 차단합니다.
