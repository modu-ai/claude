...
| 스킬 | 시점 |
|---|---|
| `moai-cowork:higgsfield-video` | 후속: 이미지를 영상으로 |
| `moai-cowork:gemini-3-image-prompt` | 대안: 프롬프트만 산출 (외부 도구) |
| `moai-cowork:gpt-image-2-prompt` | 대안: 외부 ChatGPT 사용 |
| `moai-cowork:codex-image` | 대안: codex CLI gpt-image-2 (OAuth, API 키 불필요·로컬) |
| `moai-cowork:card-news` | 후속: 이미지를 카드뉴스에 배치 |
| `moai-cowork:cd-system-prep` | 보조: 브랜드 톤 확정 |
| `moai-cowork:campaign-planner` | 보조: 캠페인 시리즈 이미지 |
