...
PM은 두 가지 모드로 일합니다. `--cowork` 모드(기본값 — 플래그를 생략하면 이 모드)는 개발을 제외한 모든 협업 프로젝트용으로, 전 직원 지침 설정과 `.claude/agents/` 커스텀 에이전트, CLAUDE.md(클로드가 프로젝트마다 읽는 지침 파일) 워크플로우까지 생성하고 스스로 개선해 나갑니다. `--code` 모드는 코더의 개발 환경 셋업(`moai init` 동등 효과)을 담당할 예정으로, **현재 개발 중**입니다. 비개발자도 자연어 한마디로 프로젝트를 시작할 수 있게 하는 것이 존재 이유입니다.
