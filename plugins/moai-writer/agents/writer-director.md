...
5. **Observe** — Check the output against the skill's own quality bar and the author's stated constraints (genre conventions, manuscript length in 200자 원고지, submission form requirements). For Korean prose, chain finishing: `book-revision-coach` → `korean-spell-check` → `general-humanize-korean`.
