...
`modu-ai/claude`는 GitHub 저장소 주소의 줄임 표기입니다. 성공하면 "Added marketplace: moai-claude" 같은 확인 메시지가 출력되고, 이제 Claude가 이 가게의 플러그인 목록을 알게 됩니다. 등록된 마켓플레이스는 다음 명령으로 확인할 수 있습니다.
...
Claude Desktop 앱에서는 설정 → 플러그인(Plugins) 화면에서 마켓플레이스 주소로 `modu-ai/claude`를 추가하면 같은 결과를 얻습니다.

{{< screenshot-request "Claude Desktop 앱의 설정 > 플러그인 화면 — 마켓플레이스 추가 입력란과 등록된 moai-claude 마켓플레이스가 보이는 화면" >}}
...
```bash
claude plugin details moai-seller@moai-claude
```

`details`는 특정 플러그인의 상세 정보 — 버전, 설명, 포함된 명령·에이전트·스킬 목록, MCP 서버 선언 — 를 보여 줍니다. 설치 직후 한 번 훑어보면 이 직원이 어떤 도구를 들고 왔는지 감이 잡힙니다.

{{< screenshot-request "claude plugin details moai-seller@moai-claude 실행 결과 — 버전·스킬·MCP 서버 목록이 출력된 터미널 화면" >}}
...
claude plugin marketplace update moai-claude
claude plugin update moai-seller@moai-claude

# 잠시 쉬게 하기(삭제 없이 비활성화) / 다시 출근시키기
claude plugin disable moai-seller@moai-claude
claude plugin enable moai-seller@moai-claude

# 완전히 내보내기(제거)
claude plugin uninstall moai-seller@moai-claude
```
