...
8-Phase 워크플로우(인터뷰→인벤토리→체인설계→Gap→확인→CLAUDE.md→API키→첫실행). 실무/글쓰기 작가 역할 자동 감지. moai-coworker 스킬 체인 프리셋. Phase 3 입력 3종(인터뷰+인벤토리+수집 맥락 분석) + 체인별 설계 근거 기록. §3.5 인용·저작권 가드 HARD(content/book/story 체인).

#### 3. designer-setup.md — `/project --designer` 분기 (디자이너 브랜드 셋업)
5-Phase 워크플로우(자산인터뷰→설치확인→DESIGN.md합성→brand스캐폴드→온보딩안내). cd-system-prep + moai-domain-brand-design 위임. `.moai/project/brand/` 산출. §3-1 파일 생성 기준 + §3-2 맥락 적용 규칙(brand 컨텍스트) 흡수.

#### 4. coder-setup.md — `/project --code` 분기 (코더 개발환경 셋업)
5-Phase 워크플로우(유형인터뷰→설치확인→정본스캐폴드→언어·MCP→SPEC안내). moai-workflow-project 정본 templates로 MoAI-ADK 3.0 baseline(`.claude/`·`.moai/`·CLAUDE.md) 설치. Phase 3-1 두-템플릿 분리 HARD(PM 공통 tmpl 코더 분기 미적용).
...
맥락 충분성 등급(A/B/C), 모호성 감지, AskUserQuestion 전략. §5-3 맥락 적용 규칙 HARD(선택 적용·자연스러운 반영·메타 코멘터리 금지 문구 목록).

#### 7. claudemd-generator.md — CLAUDE.md 생성 프로토콜
`references/templates/CLAUDE.md.tmpl` 변수 치환 규칙, 200라인 예산(신규 블록 5종 재배분), HARD 규칙 고정 블록 7종(office 우선·ai-slop + 요청 평가 사다리·파일 생성 기준·인용·저작권 가드·톤 규칙·맥락 적용 규칙 — 초과 시 축소는 체인만).

#### 8. execution-protocol.md — 스킬 체인 실행 프로토콜
플러그인 트리거 → 체인 순차 실행 → 단계별 요약 → general-ai-slop-reviewer 종료. §6-5 검색 스케일링(1회/3-5회/5-10회+ — §6-4 검증 깊이 사다리 정렬).
...
- `CLAUDE.md.tmpl` — coworker 분기 Phase 6에서 로드되는 CLAUDE.md 생성 템플릿. HARD 규칙 블록 7종 고정(office 우선·ai-slop + 요청 평가 사다리·파일 생성 기준·인용·저작권 가드·톤 규칙·맥락 적용 규칙), 정적 ≤150라인. 코더 분기에는 적용하지 않는다(두-템플릿 분리 — coder-setup.md Phase 3-1).
