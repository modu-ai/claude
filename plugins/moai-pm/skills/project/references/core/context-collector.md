...
맥락 충분성 등급, 심화 인터뷰, 모호성 감지, 반복 제한을 통해 효율적인 수집을 구현하고, 수집된 맥락의 **적용 규칙**(§5-3)까지 정의합니다.
...
### 5-3. 맥락 적용 규칙 (HARD — 수집된 맥락의 사용)

§5-1에 저장된 맥락(CLAUDE.md 프로젝트 개요 + `.moai/context.md` + 세션 context_store)을 **사용**할 때의 규칙. 수집과 저장이 아무리 좋아도 적용이 어색하면 산출물 품질과 몰입이 무너진다.

1. **선택적 적용** — 맥락은 현재 요청과 관련 있을 때만 골라 쓴다. 일반 지식 질문에 프로젝트 맥락을 억지로 결부하지 않는다.
2. **자연스러운 반영** — 적용할 때는 원래 알던 것처럼 녹인다. 어디서 가져왔는지 티 내지 않는다.
3. **메타 코멘터리 금지** — 맥락 회수를 기계적으로 서술하는 문구를 응답·산출물에 쓰지 않는다. 금지 문구 목록(유형 예시):
   - "저장된 맥락에 따르면 …"
   - "제가 기억하기로는 …"
   - "프로젝트 개요를 보니 …"
   - "이전에 말씀하신 내용을 찾아보니 …"
   - "context.md에 기록된 바로는 …"
4. **재진입(resume) 세션 주의** — 설치 후 `/project resume`로 돌아온 세션은 캐시된 맥락을 다시 읽는 순간이라 메타 코멘터리가 특히 빈발한다. 재개 인사에서도 맥락은 자연스럽게만 반영한다.

이 규칙은 Phase 6 생성 CLAUDE.md의 `맥락 적용 규칙` HARD 블록과 쌍이다(`claudemd-generator.md` 참조) — 셋업 시점(본 파일)과 런타임(생성 CLAUDE.md) 양쪽에서 같은 규칙이 적용된다.
