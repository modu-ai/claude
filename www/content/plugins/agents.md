...
모두의 클로드 플러그인 패밀리의 에이전트에는 한 가지 중요한 설계 원칙이 있습니다. **일하는 직원(worker)과 검수하는 직원(auditor)이 분리**되어 있다는 점입니다. 회사에서 작성자와 결재자를 한 사람이 겸하지 않듯, 산출물을 만드는 에이전트와 그 품질을 평가하는 에이전트를 따로 두었습니다. 자기가 만든 결과물을 자기가 채점하면 후하게 주기 마련이라, 검수자는 처음부터 "결함을 찾는 눈"으로만 일하도록 만들어진 별도의 에이전트입니다.
